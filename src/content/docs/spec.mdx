...
- An agent MAY send one or more signals **proactively**, as `Sec-MS` request headers — each identifying the signal by its **schema URI** (§3.2) and carrying its payload — without any prior negotiation. A time-sensitive or legally binding signal such as GPCv2 can therefore ride the very first request.
...
On the wire a signal is identified not by its signaltype name (§2) but by its **schema URI** — the URL of the signal's **definition**. The definition is the source of truth for what a signal means and what shape its payload takes; it MAY be a JSON Schema document or a human-readable specification page. The definition's contents and semantics belong to the signaltype and are out of scope of this specification.

A schema URI is **immutable**: a revised definition MUST be published at a new URL (for example `/defs/myterms-v2.json`) rather than by mutating the existing resource. Because a definition cannot change in place, an agent MAY cache it indefinitely (§3.6).
...
- **Privacy**: Provide legally binding notice to the service provider that it must respect your right to require that it “Do Not Sell” your personal information. MySignals provides an alternative mechanism to convey the Global Privacy Control [[**GPC**]](#ref-gpc) signal semantic.
...
Each entry in the `signals` map carries a required `definitionUri` — the SDK-side schema URI for the signal (§3.2). It may point to a JSON Schema document or a human-readable spec page, and as at the protocol level is treated as immutable: publish new versions at new URLs, e.g. `/schemas/myterms/v2.json`.
