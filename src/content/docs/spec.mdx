...
enable: true
title: "MySignals"
subtitle: "Editor's Draft: 05 June 2026"
...
abstract: "This document defines a handshake process wherein a person and a service provider (or other person) exchange signals to negotiate the terms of their interactions. The handshake is conducted between the person's agent and the service provider’s website/app. Signals may be sent on the first request without prior negotiation; discovery of a service provider's supported signals is optional, in-band, and cacheable; and each received signal is acknowledged by the service provider."
status:
  boxes:
    - type: "advisement"
...
MySignals is a handshake process between a person's agent and a service provider website/app. It is an extensible communications framework that allows developers to define specific kinds of signals (signaltypes) that can be exchanged. It defines a common namespace for these signaltypes and a syntax for passing parameters. It has these characteristics:
...
- An agent MAY send one or more signals **directly**, as `Sec-MS` request headers — each identifying the signal by its **schema URI** (§5) and carrying its payload — without any prior negotiation. A time-sensitive or legally binding signal such as GPCv2 can therefore ride the very first request.
...
- Discovery follows a pattern similar to that used in [[**ClientHints**]](#ref-clienthints), reducing the fingerprinting surface area and network traffic.
- It supports structured, multi-parameter signals carried in the payloads.
- It supports HTTP/websites. Support for mobile apps is anticipated in a future version of this specification.

## 2. Definitions

A **signaltype** is a human-readable name for a kind of signal a person can send to a service provider (see §4). On the wire a signal is not identified by this name but by its **schema URI** — the URL of the signal's definition (§5). Implementers of this spec MUST use the signaltypes defined in §4. The semantics of each signaltype are defined by an implementer — they are outside of the MySignals spec.

A signal carries a **payload**: value whose shape is described by the signal's schema. The payload's contents and semantics are defined by the signaltype's definition and are out of scope of the MySignals spec.

## 3. Handshake Process

The framework is a stateless exchange of three independent parts: an agent **submits** one or more signals, MAY first **discover** what a service provider supports, and receives a per-signal **acknowledgement**. Discovery is not a prerequisite for submission.

### 3.1 Statelessness

The framework establishes no session. A service provider MUST NOT require a prior probe before honouring a submitted signal, and MUST NOT depend on remembering that any particular agent has probed before. Each request is interpreted on its own. This keeps the protocol correct across CDNs, retries, parallel tabs, and load-balanced origins. Any continuity an agent maintains (for example the SDK's stored handshake state, §6.2) is a purely client-side optimisation, not service-provider state.

### 3.2 Sending signals

An agent submits a signal by including a `Sec-MS` request header whose value is `<schemaUri>;<payload>` — the signal's schema URI (§5), a semicolon, then the payload (a scalar such as a boolean opt-out, or a structured JSON value, or any arbitrary strucuture defined in a schema). One `Sec-MS` header is sent per signal; repeat the header to send several. Signals MAY be sent on the very first request, without any prior discovery:

```http
GET /page HTTP/2
Host: example.com
Sec-MS: https://example.com/defs/gpcv2.json;1
Sec-MS: https://example.com/defs/myterms-v1.json;{"contract":"P7012-DDA-1"}
```

```http
HTTP/1.1 200 OK
Ack-MS: https://example.com/defs/gpcv2.json=Ack
Ack-MS: https://example.com/defs/myterms-v1.json=Ack
```
...
<Callout type="note" title="Sec- vs X- headers">
  ...
  and `X-MS-Discovery` headers (§6.3). A service provider treats the `Sec-MS`
...
```http
HTTP/1.1 200 OK
Accept-MS: https://example.com/defs/gpcv2.json, https://example.com/defs/myterms-v1.json
...
**Recognised but invalid** — for example an out-of-range value: GPCv2 expresses a single boolean opt-out, so `2` is not valid and fails its schema:

```http
Sec-MS: https://example.com/defs/gpcv2.json;2
```

```http
Ack-MS: https://example.com/defs/gpcv2.json=BadData
```
...
```http
Sec-MS: https://example.com/defs/gpcv2.json;1
Sec-MS: https://example.com/defs/myterms-v1.json;{}
Sec-MS: https://example.com/defs/siopv2.json;{"client_id":"did:example:123"}
```

```http
Ack-MS: https://example.com/defs/gpcv2.json=Ack
Ack-MS: https://example.com/defs/myterms-v1.json=BadData
Ack-MS: https://example.com/defs/siopv2.json=Unknown
```
...
### 3.5 Caching and signal discovery

Discovery is intended to be inexpensive through caching at three layers:

- **Discovery results.** The set of schema URIs advertised in `Accept-MS` MAY change over time and SHOULD therefore be cached with a finite freshness lifetime and revalidation. A service provider SHOULD return standard HTTP caching directives — `Cache-Control` (`max-age`), `ETag`, and optionally `stale-while-revalidate`. An agent SHOULD associate the advertised set with the origin and honour those directives.
- **Definition resources.** A schema URI is immutable: a revised definition MUST be published at a new URL (for example `/defs/myterms-v2.json`) rather than by mutating the existing resource (§5). Because a definition cannot change in place, an agent MAY cache it indefinitely, and a service provider SHOULD serve it with `Cache-Control: public, max-age=31536000, immutable`.
- **Agent-side storage.** An agent MAY persist discovery results locally to avoid redundant probes. The JS SDK implements this with selectable `memory`, `sessionStorage`, and `localStorage` (24-hour TTL) strategies (§6.2).
...
The MySignals JS SDK is a zero-dependency JavaScript/TypeScript library that implements the handshake flow on any web page. It ships as ESM, CommonJS, and UMD, runs in modern bundlers (Vite, Webpack, Rollup) and directly in the browser via a `<script>` tag, and relies only on native APIs such as `fetch`.

### 5.1 API and lifecycle

`MySignalsClient` exposes a small lifecycle API: `init()` probes the server for `CRITICAL` and `ACCEPTED` signal requirements, `submit()` delivers the user's signals, `getStatus()` returns a snapshot of state, pending requirements, and errors, and `reset()` clears session state and storage. The handshake is modeled as a finite state machine:
...
- **Storage**: persists handshake state in `memory`, `sessionStorage`, or `localStorage` (with a 24-hour TTL) to avoid redundant prompting.
...
- **Cross-origin**: performs the handshake across domains, optionally attaching credentials/cookies to `fetch` requests.

### 5.3 Signal definitions and validation

Each entry in the `signals` map carries a required `definitionUri` — the source of truth for what a signal means and what shape its value takes. It may point to a JSON Schema document or a human-readable spec page, and is treated as immutable (publish new versions at new URLs, e.g. `/schemas/myterms/v2.json`).

Validation is advisory by default; the server remains authoritative. If no validator is configured the SDK skips fetching `definitionUri`. Set `strictValidation: true` to reject locally with `MySignalsErrorCode.SCHEMA_VIOLATION`. The SDK stays dependency-free via a `SchemaValidator` interface — bring your own (Ajv example):
...
On the wire, the SDK sends each signal as an `X-MS` request header (value `<schemaUri>;<payload>`) rather than `Sec-MS`, and probes with `X-MS-Discovery` rather than `Sec-MS-Discovery`: `Sec-` is a forbidden header name that page scripts cannot set, so the `X-MS` and `X-MS-Discovery` headers are the script-accessible equivalents of the canonical user-agent forms (§3.2, §3.3), which service providers honour identically. Payloads are JSON-encoded — strings pass through unchanged; objects, booleans, and numbers are `JSON.stringify`-ed — and schemas describe the value as the developer sees it, not the wire form. The SDK surfaces the service provider's per-signal `Ack-MS` results (`Ack` / `Unknown` / `BadData`, §3.4) to the caller.
